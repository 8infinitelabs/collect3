...
func (db *SQLiteRepository) CreateContent(cid string) (error) {
	res, err := db.db.Exec("INSERT INTO content(cid) values(?)", cid)
	if err != nil {
		var sqliteErr sqlite3.Error
		if errors.As(err, &sqliteErr) {
			if errors.Is(sqliteErr.ExtendedCode, sqlite3.ErrConstraintUnique) {
...
func (db * SQLiteRepository) UploadContent(uid string, cid string) error {
  var err error
